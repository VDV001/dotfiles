        ...
        mcpServers = {
          ...
          kb = {
            ...
            args = [
              ...
              # Адрес витрины для поля view: инструмент отвечает двоим — агенту
              # данными, человеку ссылкой на ту же выборку, чтобы сверять ответ
              # по первичным данным, а не по пересказу модели.
              #
              # ⚠️ Без этого флага поле приходит ПУСТЫМ по конструкции, и вся
              # работа со ссылками (#322) вместе с меткой «ответ агента» на
              # витрине (#324) не видна вовсе — замерено 28.08: флага здесь не
              # было с самого выпуска.
              #
              # ⚠️ Порт тот же, что у `kbweb` (modules/kbengine.nix): витрина
              # поднимается по требованию, поэтому ссылка мертва, пока её не
              # запустили. Размен сознательный — «ссылка, которая заработает
              # после kbweb» полезнее пустого поля, потому что она хотя бы
              # называет, куда идти. Второе место с числом порта — цена;
              # разъедутся они молча.
              "--view-base"
              "http://127.0.0.1:8097"
            ];
            type = "stdio";
